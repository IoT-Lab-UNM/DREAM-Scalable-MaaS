#!/usr/bin/env bash
set -euo pipefail

echo "========== Edge node base setup (KubeEdge / edge runtime) =========="

echo "[1/8] Checking OS/kernel..."
uname -r
if command -v nft >/dev/null 2>&1; then
  nft --version
else
  echo "nft command not found. Continuing..."
fi

echo "[2/8] Disabling swap..."
sudo swapoff -a
sudo sed -i.bak '/\sswap\s/s/^/#/' /etc/fstab || true

echo "[3/8] Loading required kernel modules..."
cat <<EOF | sudo tee /etc/modules-load.d/k8s.conf
overlay
br_netfilter
EOF

sudo modprobe overlay
sudo modprobe br_netfilter

echo "[4/8] Applying sysctl settings..."
cat <<EOF | sudo tee /etc/sysctl.d/k8s.conf
net.bridge.bridge-nf-call-iptables = 1
net.bridge.bridge-nf-call-ip6tables = 1
net.ipv4.ip_forward = 1
EOF

sudo sysctl --system

echo "[5/8] Verifying settings..."
lsmod | grep -E 'overlay|br_netfilter' || true
sysctl net.bridge.bridge-nf-call-iptables
sysctl net.bridge.bridge-nf-call-ip6tables
sysctl net.ipv4.ip_forward

echo "[6/8] Removing conflicting packages..."
sudo apt-get remove -y docker.io docker-compose docker-compose-v2 docker-doc podman-docker containerd runc || true

echo "[7/8] Installing dependencies and containerd..."
sudo apt-get update
sudo apt-get install -y ca-certificates curl gpg

sudo install -m 0755 -d /etc/apt/keyrings
sudo curl -fsSL https://download.docker.com/linux/ubuntu/gpg -o /etc/apt/keyrings/docker.asc
sudo chmod a+r /etc/apt/keyrings/docker.asc

sudo tee /etc/apt/sources.list.d/docker.sources <<EOF
Types: deb
URIs: https://download.docker.com/linux/ubuntu
Suites: $(. /etc/os-release && echo "${UBUNTU_CODENAME:-$VERSION_CODENAME}")
Components: stable
Signed-By: /etc/apt/keyrings/docker.asc
EOF

sudo apt-get update
sudo apt-get install -y containerd.io

sudo mkdir -p /etc/containerd
sudo containerd config default | sudo tee /etc/containerd/config.toml > /dev/null
sudo sed -i 's/SystemdCgroup = false/SystemdCgroup = true/' /etc/containerd/config.toml

echo "[8/8] Restarting and enabling containerd..."
sudo systemctl daemon-reload
sudo systemctl enable containerd
sudo systemctl restart containerd
sudo systemctl status containerd --no-pager || true

echo "========== Edge setup completed successfully =========="
echo "Next: continue with KubeEdge setup on this edge node."